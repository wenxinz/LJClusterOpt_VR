...
namespace DaydreamElements.Teleport {
  ...
  public class TeleportController : MonoBehaviour {
    ...
    /// Trigger used to activate forward motion.
    [Tooltip("Trigger used to signal forward motion")]
    public BaseActionTrigger forwardMotionTrigger;

    /// Trigger used to activate backward motion.
    [Tooltip("Trigger used to signal backward motion")]
    public BaseActionTrigger backwardMotionTrigger;
    ...
    /// Move player forward/backward when they click the upper/lower side of the d-pad
    [Tooltip("Move the player forward/backward when they click the upper/lower side of the touchpad")]
    public bool allowForwardBackward = true;
    ...
    void Update() {
      ...
      // If allowFowardBackward
      if (allowForwardBackward){
        HandlePlayerForwardBackward();
      }
    ...
    private Vector3 deltaMovement = new Vector3(0, 0, 0.5f);
    
    private void HandlePlayerForwardBackward(){
       // Move forward
       if(forwardMotionTrigger != null && forwardMotionTrigger.TriggerActive()) {
          player.position += deltaMovement;
       }

       // Move backward
       if(backwardMotionTrigger != null && backwardMotionTrigger.TriggerActive()) {
          player.position -= deltaMovement; 
       }
